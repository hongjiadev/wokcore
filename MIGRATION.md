...
- Deliberate adaptation:
  ...
  - publish preparation garbage-collects the fixed `.wokcore-publish-staging` entry and strict platform-specific `.wokcore-tombstone-*` internal format only after no-follow regular-file and owner-only permission checks; identity-encoded tombstones additionally require the encoded file identity to match. Windows uses one fixed publish staging entry, so abrupt termination after its file synchronization but before commit leaves at most that one recoverable object; the next lease-owned preparation opens and validates the exact staging entry before deleting it, while ordinary returned failures retain `NamedTempFile`/`TempPath` RAII cleanup. Malformed, wrong-type, reparse, or identity-mismatched internal entries fail closed and are retained. These internal names are private WokCore garbage inside the `0700`/current-user-only runtime directory: same-UID adversarial swaps after an internal garbage-name check are outside the cleanup contract, but cleanup never targets canonical `discovery.json`, follows links, or reaches outside the runtime directory. Completed publish/remove lifecycles retain at most one internal entry, and steady reads perform no cleanup or other writes;
