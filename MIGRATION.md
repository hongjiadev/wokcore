...
- Deliberate adaptation:
  - runtime ownership uses a non-blocking `fs4` operating-system lock held by `RuntimeLease`; raw Unix child opens atomically set close-on-exec, stale lock-file text is never interpreted as ownership, and neither stale text nor an executed unrelated child extends the lease;
  ...
  - discovery publication carries the prevalidated destination identity through replacement: Unix atomically moves an existing canonical entry to an identity-encoded internal tombstone and installs the synchronized temporary entry with directory-FD-relative no-clobber renames, while Windows keeps verified directory/destination handles that deny delete sharing, moves the exact destination handle aside, and installs without replacement; an entry installed after the precheck is preserved and causes publication to fail closed;
  ...
  - publish preparation garbage-collects only the strict platform-specific `.wokcore-tombstone-*` internal format after no-follow regular-file, owner-only permission, and encoded file-identity checks; malformed, wrong-type, or identity-mismatched entries fail closed and are retained. This garbage namespace is private to WokCore inside the `0700`/current-user-only runtime directory: same-UID adversarial swaps after an internal garbage-name check are outside the cleanup contract, but cleanup never targets canonical `discovery.json`, follows links, or reaches outside the runtime directory. Completed publish/remove lifecycles retain at most one internal tombstone, and steady reads perform no cleanup or other writes;
