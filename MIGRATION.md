...
- SQLite schema 5 expands that exact allow-list with `service.read`, `service.control`, `providers.read`, `providers.write`, and `clients.manage` while preserving existing token rows unchanged.
...
- Client authorization supports exact repeatable scopes: `proxy.use`, `sessions.read`, `usage.read`, `diagnostics.read`, `diagnostics.export`, `service.read`, `service.control`, `providers.read`, `providers.write`, and `clients.manage`. Existing schema-2 client tokens retain only `proxy.use`.
...
- Every Provider endpoint accepts the management token for compatibility. Exact `providers.read` and `providers.write` client scopes split read and mutation access; `proxy.use` alone grants neither. Methods, JSON content types, body bounds, paths, and revision conflicts fail closed with stable content-free errors.
...
- OpenAPI 3.1 documents all 21 control-plane and seven data-plane operations, management-token compatibility, every exact client scope, JSON/SSE/multipart media types, and 16/20/50/51 MiB limits without credential values or token samples.
