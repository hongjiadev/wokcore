...
- Deliberate adaptation:
  ...
  - discovery publication retains the reviewed atomic-file primitives, anchors Unix child opens, temporary creation, and replacement to the verified runtime-directory handle, and synchronizes that opened parent-directory handle after replacement on Windows as well as Unix; configuration revisions, daemon PID liveness inference, fallback ports, legacy control IPC, and WokRouter runtime lifecycle behavior are not imported;
  - Unix owned removal atomically quarantines the current directory entry through the verified runtime-directory handle, verifies the quarantined file identity, deletes only a match, restores a mismatch only while the canonical name remains absent, and never overwrites a newer canonical entry.
