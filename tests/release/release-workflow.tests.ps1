...
if ($cargo -notmatch '(?ms)\[workspace\.package\].*?version\s*=\s*"0\.1\.5"') {
    throw "Workspace package version must be 0.1.5."
}
if (
    $source -notmatch
...
$contracts = @(Get-WokCoreTargetContracts -Version "0.1.5")
if ($contracts.Count -ne 6) { throw "Expected six WokCore targets." }
$payloads = @(Get-WokCorePayloadNames -Version "0.1.5" -IncludeLegacyV1)
