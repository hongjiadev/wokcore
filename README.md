...
Health and the versioned capability handshake are public. Every other control-plane operation accepts the management Bearer token for bootstrap and compatibility, or a client token with its exact required scope. The management token is referenced by local discovery and resolved through the configured secret backend. Raw client tokens are returned only by a successful authorize response and are never recoverable from SQLite or discovery.
...
Client authorization supports ten exact scopes without wildcards or implicit expansion: `proxy.use`, `sessions.read`, `usage.read`, `diagnostics.read`, `diagnostics.export`, `service.read`, `service.control`, `providers.read`, `providers.write`, and `clients.manage`. Session, usage, log, diagnostic-export, service, Provider, and client-management endpoints accept either the management token or a client token with the corresponding scope. Diagnostic events are bounded and redacted; ordinary request events remain in the 16 MiB memory ring, while lifecycle events, warnings, and errors are batch-persisted into rotating segments. A diagnostic export is streamed with a 64 MiB hard bound, is create-new in the CLI, and never contains Session bodies, credentials, authorization headers, cookies, raw tokens, or absolute user paths.
...
Provider catalog, runtime, and model reads require `providers.read`; Provider configuration and secret mutations require `providers.write`. The management token remains accepted for both. A client token carrying only `proxy.use` cannot read or mutate Provider state. This surface performs no OAuth browser flow, Provider discovery, DNS lookup, or upstream request.
