...
fn windows_uses_wokcore_roots_and_runtime_artifact_names() {
    ...
    assert_eq!(
        paths.config_file,
        PathBuf::from(r"C:\\Users\\Ada\\AppData\\Roaming\\WokCore\\config.toml")
    );
    assert_eq!(
        paths.state_db,
        PathBuf::from(r"C:\\Users\\Ada\\AppData\\Local\\WokCore\\state.sqlite3")
    );
    assert_eq!(
        paths.runtime_dir,
        PathBuf::from(r"C:\\Users\\Ada\\AppData\\Local\\WokCore\\runtime")
    );
    assert_eq!(
        paths.log_dir,
        PathBuf::from(r"C:\\Users\\Ada\\AppData\\Local\\WokCore\\logs")
    );
    assert_eq!(
        paths.discovery_file,
        PathBuf::from(r"C:\\Users\\Ada\\AppData\\Local\\WokCore\\runtime\\discovery.json")
    );
    assert_eq!(
        paths.instance_lock,
        PathBuf::from(r"C:\\Users\\Ada\\AppData\\Local\\WokCore\\runtime\\instance.lock")
    );
}
...
fn macos_uses_wokcore_application_support_directory() {
    ...
    assert_eq!(
        paths.config_file,
        PathBuf::from("/Users/ada/Library/Application Support/WokCore/config.toml")
    );
    assert_eq!(
        paths.state_db,
        PathBuf::from("/Users/ada/Library/Application Support/WokCore/state.sqlite3")
    );
    assert_eq!(
        paths.runtime_dir,
        PathBuf::from("/Users/ada/Library/Application Support/WokCore/runtime")
    );
}
...
fn linux_uses_absolute_xdg_roots_and_wokcore_runtime_directory() {
    ...
    assert_eq!(
        paths.config_file,
        PathBuf::from("/etc/xdg/WokCore/config.toml")
    );
    assert_eq!(
        paths.state_db,
        PathBuf::from("/var/lib/xdg/WokCore/state.sqlite3")
    );
    assert_eq!(paths.runtime_dir, PathBuf::from("/run/user/1000/WokCore"));
    assert_eq!(paths.log_dir, PathBuf::from("/var/lib/xdg/WokCore/logs"));
    assert_eq!(
        paths.discovery_file,
        PathBuf::from("/run/user/1000/WokCore/discovery.json")
    );
    assert_eq!(
        paths.instance_lock,
        PathBuf::from("/run/user/1000/WokCore/instance.lock")
    );
}
...
fn relative_linux_environment_paths_fall_back_to_home_owned_directories() {
    ...
    assert_eq!(
        paths.config_file,
        PathBuf::from("/home/ada/.config/WokCore/config.toml")
    );
    assert_eq!(
        paths.state_db,
        PathBuf::from("/home/ada/.local/state/WokCore/state.sqlite3")
    );
    assert_eq!(
        paths.runtime_dir,
        PathBuf::from("/home/ada/.local/state/WokCore/runtime")
    );
}
...
fn resolving_paths_creates_no_directories_or_files() {
    ...
    let paths = AppPaths::resolve(EnvironmentSnapshot::new(
        Platform::Windows,
        [
            ("APPDATA", app_data.as_str()),
            ("LOCALAPPDATA", local_app_data.as_str()),
        ],
    ))
