...
#[test]
fn existing_schema_one_upgrades_to_schema_five_without_losing_data() {
...
#[test]
fn concurrent_schema_one_opens_apply_schema_five_once() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("state.db");
    create_schema_one(&path);
...
fn read_only_inspection_reads_health_and_management_binding_without_writes() {
    ...
    assert_eq!(read_only.health().unwrap().schema_version, 5);
    assert_eq!(
        read_only
            .runtime_secret_binding("management")
            .unwrap()
...
fn read_only_inspection_sees_a_valid_committed_crash_wal_without_writes() {
    ...
    run_crash_wal_helper(&path, "migrate-five");
    ...
    match mode.as_str() {
        "migrate-five" => {
            ...
            connection
                ...
                .unwrap();
            connection
                .execute_batch(include_str!("../migrations/0004_provider_metadata.sql"))
                .unwrap();
            connection
                .execute_batch(include_str!("../migrations/0005_client_scopes.sql"))
                .unwrap();
        }
